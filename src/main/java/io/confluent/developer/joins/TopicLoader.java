    ...
    public static void runProducer() throws IOException {
        ...
        try (Admin adminClient = Admin.create(properties);
            ...
            try {
                ...
                User userOne = User.newBuilder().setUserId("10261998").setAddress("5405 6th Avenue")
                        .setName("Elizabeth Jones").build();
                User userTwo = User.newBuilder().setUserId("10261999").setAddress("407 64th Street")
                        .setName("Art Vandelay")
                        .build();

                var users = List.of(userOne, userTwo);

                // Users must be created first so the KTable records have earlier timestamps
                // than the KStream records,
                // otherwise the join will ignore them.
                users.forEach(user -> {
                    ProducerRecord<String, SpecificRecord> producerRecord = new ProducerRecord<>(tableTopic,
                            user.getUserId(), user);
                    producer.send(producerRecord, StreamsUtils.callback());
                });

                applianceOrders.forEach((ao -> {
                    ProducerRecord<String, SpecificRecord> producerRecord = new ProducerRecord<>(leftSideTopic,
                            ao.getUserId(), ao);
                    producer.send(producerRecord, StreamsUtils.callback());
                }));

                electronicOrders.forEach((eo -> {
                    ProducerRecord<String, SpecificRecord> producerRecord = new ProducerRecord<>(rightSideTopic,
                            eo.getUserId(), eo);
                    producer.send(producerRecord, StreamsUtils.callback());
                }));
            } catch (Exception e) {
